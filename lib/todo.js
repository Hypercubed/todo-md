...
Todo.prototype.do = function(index) {
	...
	if (md)
		this.md[index - OFFSET] = md.replace(/\[[\s\t]+\]/, '[x]');
...
Todo.prototype.undo = function(index) {
	...
	if (md)
		this.md[index - OFFSET] = md.replace(/\[[xX]+\]/, '[ ]');
...
function addCount(input, index) {
	index = (input.match(/\[[\s\txX]+\]/)) ? index + OFFSET : '';
	input = input.replace(/\[[\s\t]+\]/, colors.red);
	input = input.replace(/\[[xX]+\]/, colors.green);
